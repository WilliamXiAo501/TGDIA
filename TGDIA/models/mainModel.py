...
class DecoderBN(nn.Module):
    def __init__(self, num_features=2048, num_Bins=256, score_map_dim=8):
        ...
        self.attn1 = deformable_LKA_Attention(features // 1)
        self.attn2 = deformable_LKA_Attention(features // 2)
        self.attn3 = deformable_LKA_Attention(features // 4)
        self.attn4 = deformable_LKA_Attention(features // 8)
    ...
    def forward(self, features):
        ...
        en4 = self.attn4(en4)
        en8 = self.attn3(en8)
        en16 = self.attn2(en16)
        en32 = self.attn1(en32)
...
model = DeformableAttention(stride=1, distortionmode=True)
...
class TotalModel(nn.Module):
    def __init__(self, clip_model, gpu, max_depth, min_depth, n_bins):
        ...
        self.deformableattention = DeformableAttention(stride=1, distortionmode=True)
    ...
    def forward(self, img):
        ...
        text_features = text_features + self.gamma * text_diff # cross attention back

        visual1 = self.deformableattention(visual_embeddings)
        # B, C, H, W = visual1.shape = visual_embeddings.shape
        K = text_features.size(1)
        raw_score = torch.einsum('bchw,bkc->bkhw', visual1, text_features) 
        attn = torch.softmax(raw_score, dim=1) 
        attn_flat = attn.view(B, K, -1).transpose(1, 2) 
        image_enhanced = torch.bmm(attn_flat, text_features).transpose(1, 2).reshape(B, C, H, W)
        gate = torch.sigmoid(torch.mean(raw_score, dim=1, keepdim=True)) 
        fused_visual = gate * image_enhanced + (1 - gate) * visual1 
        visual = F.normalize(fused_visual, dim=1, p=2)
